...
knitr::opts_chunk$set(message = FALSE, error = FALSE, warning = FALSE,
                      cache = TRUE, dev.args = list(png = list(type = "cairo")))
...
measures <- c(delta = "Weight of meat used (lbs)",
              ...
              delta_norm = "Weight of meat used per patron")
```
...
coef_1 <- tidy(mod_1, conf.int = TRUE)
...
coef_1b <- tidy(mod_1b) |> bind_cols(sand_1b) |> rename(ci_lower = `2.5 %`, ci_upper = `97.5 %`)
...
ggplot(aes(x = condition, y = value, colour = stage)) +
  ...
  theme(strip.placement = "outside",
        axis.title = element_blank(),
        legend.position = "bottom")
ggsave("study_lunch_target.png", width = 7, height = 3)
```
...
coef_2 <- tidy(mod_2, conf.int = TRUE)
...
coef_2b <- tidy(mod_2b) |> bind_cols(sand_2b) |> rename(ci_lower = `2.5 %`, ci_upper = `97.5 %`)
...
ggplot(aes(x = condition, y = value)) + #, colour = stage)) +
  ...
  theme(strip.placement = "outside",
        axis.title = element_blank(),
        legend.position = "bottom")
ggsave("study_dinner.png", width = 7, height = 2.75)
```

## Lunch during self-serve
...
coef_3 <- tidy(mod_3, conf.int = TRUE)
...
coef_3b <- tidy(mod_3b) |> bind_cols(sand_3b) |> rename(ci_lower = `2.5 %`, ci_upper = `97.5 %`)
...
ggplot(aes(x = condition, y = value)) +
  ...
  theme(strip.placement = "outside",
        axis.title = element_blank(),
        legend.position = "bottom")
ggsave("self_dinner_target.png", width = 7, height = 2.75)
```
